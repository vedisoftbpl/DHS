...
public class UserFormController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(request.getParameter("userDesignation")!= null && request.getParameter("userDesignation").trim().length() > 0) {
			userDesignation = request.getParameter("userDesignation");
		}
		
		String userCreatedBy = new String();
		if(request.getParameter("userCreatedBy")!= null && request.getParameter("userCreatedBy").trim().length() > 0) {
			userCreatedBy = request.getParameter("userCreatedBy");
		}
		
		//System.out.println(userCreatedBy + "1111");
		String userFormAddress = new String();
		if(request.getParameter("userFormAddress")!= null && request.getParameter("userFormAddress").trim().length() > 0) {
		...
		if (operation.equals("create")) {
			...
			Users u = new Users(userName, userEmail, userPassword, userDesignation, userMobile, userStatus, doj, dob , userFormAddress, userPhoto , userCreatedBy, userType);
			...
			//RequestDispatcher rd = request.getRequestDispatcher(page + "?msg=1");
